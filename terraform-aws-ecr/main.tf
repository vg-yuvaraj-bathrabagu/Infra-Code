...
resource "null_resource" "build_docker_image_push" {
  depends_on = ["aws_ecr_repository.default"]
  provisioner "local-exec" {
    command = "sh build.sh ${var.region} ${var.access_key} ${var.secret} ${join("", aws_ecr_repository.default.*.repository_url)} ${var.name}"
  }
}
